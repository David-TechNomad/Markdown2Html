...
const bindHotkeys = (content, dialog) => isPlatformWindows ? {
  "Ctrl-B": () => {
  ...
  "Ctrl-Alt-S": () => {// Converting between sans serif and serif
  },
  "Ctrl-Alt-L": () => {
  ...
  "Cmd-Alt-S": () => {// Converting between sans serif and serif
  },
  "Cmd-Alt-L": () => {
    keyEvents.parseLinkToFoot(content.content, content);
  },
...
export const rightClick = cm => {
  ...
  ele.oncontextmenu = e => {
    ...
    element.style.display = "block"; // event--ie  ev--其他浏览器

    const oEvent = window.event || window.ev; // documentElement--其他游览器    body--谷歌

    const scrollTop = document.documentElement.scrollTop || document.body.scrollTop; // 菜单的style样式跟随鼠标的位置
