...
export const markdownParserWechat = new MarkdownIt({
  ...
  highlight: (str, lang) => {
    ...
    return '<section class="code-snippet__fix code-snippet__js">' + '<ul class="code-snippet__line-index code-snippet__js">' + numbers.join("") + "</ul>" + '<pre class="code-snippet__js" data-lang="' + lang + '">' + codeLines.join("") + "</pre></section>";
  }
});
markdownParserWechat.use(markdownItSpan) // 在标题标签中添加span
...
.use(markdownItImsize); // 普通解析器，代码高亮用highlight
...
export const markdownParser = new MarkdownIt({
  ...
  highlight: (str, lang) => {
    ...
    if (lang && highlightjs.getLanguage(lang)) {
      try {
        const formatted = highlightjs.highlight(lang, str, true).value.replace(/\n/g, "<br/>") // 换行用br表示
...
markdownParser.use(markdownItSpan) // 在标题标签中添加span
.use(markdownItTableContainer) // 在表格外部添加容器
.use(markdownItMath) // 数学公式
.use(markdownItLinkfoot) // 修改脚注
.use(markdownItTableOfContents, {
