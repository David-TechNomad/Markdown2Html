...
import { SM_MS_PROXY, ALIOSS_IMAGE_HOSTING, QINIUOSS_IMAGE_HOSTING, GITEE_IMAGE_HOSTING, GITHUB_IMAGE_HOSTING, IMAGE_HOSTING_TYPE, IS_CONTAIN_IMG_NAME, IMAGE_HOSTING_NAMES } from "./constant";
...
export const qiniuOSSUpload = async ({
  ...
  try {
    ...
    base64Reader.onload = e => {
      ...
      const fileType = urlData.split(";").shift().split(":").pop(); // base64转blob
      ...
      const OSSName = getOSSName(file.name, namespace); // 这里第一个参数的形式是blob

      const imageObservable = qiniu.upload(blob, OSSName, token, putExtra, conf); // 上传成功后回调
...
export const smmsUpload = ({
  ...
  showUploadNoti(); // SM.MS图床必须这里命名为smfile
...
const aliOSSPutObject = ({
  ...
  client.put(OSSName, buffer).then(response => {
    const names = file.name.split(".");
    names.pop();
    const filename = names.join(".");
    const image = {
...
export const aliOSSUpload = ({
  ...
  base64Reader.onload = e => {
    ...
    const blob = toBlob(base64, fileType); // blob转arrayBuffer
...
export const githubUpload = ({
  ...
  base64Reader.onload = e => {
    ...
    axios.put(url, data, {
      ...
      names.pop();
      const filename = names.join(".");
      const imageUrl = config.jsdelivr === "true" ? `https://cdn.jsdelivr.net/gh/${config.username}/${config.repo}/${dir}/${dateFilename}` : response.content.download_url;
      const image = {
        filename,
        url: imageUrl
      };
...
export const uploadAdaptor = (...args) => {
  ...
  if (type === userType) {
    ...
    if (!config.region.length || !config.accessKey.length || !config.secretKey.length || !config.bucket.length || !config.domain.length) {
    ...
    if (!config.region.length || !config.accessKeyId.length || !config.accessKeySecret.length || !config.bucket.length) {
