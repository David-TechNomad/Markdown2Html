...
function linkFoot(state, silent) {
  ...
  if (!silent) {
    ...
    if (title) {
      ...
      const footnoteId = state.env.footnotes.list.length; // *用来让链接倾斜
...
export default (md => {
  ...
  md.renderer.rules.footnote_close = renderFootnoteClose; // helpers (only used in other rules, no tokens are attached to those)
