...
export const MaxLocalDocumentLength = 30; // export const AutoSaveInterval = 10 * 60 * 1000;
...
export const setLocalDocuments = (db, localDocuments, document = {}) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const store = transaction.objectStore("customers");
    let req = {}; // Info: 长度超过用 put，没超过用 add

    if (localDocuments.length >= MaxLocalDocumentLength) {
      const {
        id
      } = localDocuments.sort(saveTimeSort)[localDocuments.length - 1];
      req = store.put({ ...document,
        id
      });
    } else {
...
export const setLocalDraft = (db, localDocuments, document = {}) => {
  ...
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(["customers"], "readwrite");
    const store = transaction.objectStore("customers"); // Info: 更新草稿
